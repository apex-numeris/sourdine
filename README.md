...
reste **indétectable** (cf. ACSAC 2022 sur le full replay) → suppression résiduelle assumée. En
...
versionnés (citables) : `samples/example-0.12.0.json` (dernier) et ses
prédécesseurs (`example-0.{1..11}.0.json`), conservés comme historique.
...
> deux cas.
>
> **Constats de fidélité (run docker v0.12.0)** — surfacés en exécutant la vraie
...
> Bilan docker : **87,5 / 81,0 / 10,5 / 16,7 / 100 %** (suppression / rattrapage / FP /
> résiduel / cohérence) vs sim **100 / 83,3 / 10,5 / 16,7 / 100 %**. Exemple :
> `samples/example-0.12.0-docker.json`. Le **flapping** (dont sur `jailbreak_rate`)
...
l'échantillon gelé `samples/example-0.12.0.json` (agrégat **et** chaque scénario ;
...
`samples/example-0.12.0-docker.json`. Il monte une vraie cible éphémère (~6-18 min) et
...
```bash
python3 run_campaign.py --backend sim    --out samples/example-0.12.0.json --quiet
python3 run_campaign.py --backend docker --out samples/example-0.12.0-docker.json --quiet   # ~6-18 min
git diff samples/
```
...
│   ├── SCHEMA.md
│   ├── attacks/*.json           # 20 vecteurs (24 scénarios d'attaque)
│   └── healthy/*.json           # cohérence + pièges à faux positif (19 sains)
...
└── samples/                     # exemples versionnés cumulés : example-0.{1..12}.0.json (sim) + *-docker.json
```
