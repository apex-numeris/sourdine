...
## 2. Cartographie des modules (`engine/`)
...
| `target_sim.py` | Cible **sim** : interprète l'intention du scénario en un état final déterministe |
...
| `scenarios.py` | Chargeur du jeu de scénarios (JSON vers `Scenario`) |
...
2. **Fenêtre métrique propre** : `query_range` part de `t0` (début du scénario),
   ...
   scénario sur l'autre (bug corrigé en `df65430`).
...
5. **Rejeu/gel (`stale_replay`, ACSAC 2022) masque dans les deux backends, rattrapage docker
   ...
   franchissement bref a été scrapé (visibilité timing-dépendante), d'où le classement TIMING_SENSITIVE.
...
7. **Watchdog suppression (`watchdog_suppression`, MITRE T1562), AUCUN écart** : le heartbeat
   ...
   de façon déterministe dans les deux backends, classé STRONG_MASK.
8. **Bombe de cardinalité (`cardinality_flood`, MITRE Impair Defenses / T1499), AUCUN écart** :
   ...
   deux backends, classé STRONG_MASK.
9. **Règle d'inhibition rogue (`rogue_inhibitor`, MITRE T1562.001), AUCUN écart** : masquage
   ...
   classé STRONG_MASK. Le pendant sain (`benign_maintenance_inhibition`), source non sanctionnée inhibant
...
10. **Panne induite du garde (`guard_outage_cover`, MITRE T1562.001), AUCUN écart** : c'est le seul
    ...
    (`guard_down_under_threat`) dans les deux backends, classé STRONG_MASK. Le pendant sain
...
11. **Périmètre d'inhibition élargi (`inhibition_scope_creep`, MITRE T1562.001), AUCUN écart** :
    ...
    masqué ET rattrapé (`inhibition_scope_creep`) dans les deux backends, classé STRONG_MASK. Le
...
12. **Silence pré-positionné (`preloaded_silence`, MITRE T1562.001), AUCUN écart** : le silence est
    ...
    silence parfaitement ordinaire, et c'est là tout le point : au moment de sa pose il est
    indiscernable d'une maintenance. Masqué ET rattrapé (`silenced_under_threat`) dans les deux
    backends, classé STRONG_MASK. Le pendant sain (`benign_maintenance_silence_op`) produit le **même
